...
app.provider('mobile', function() {
    ...
    return {
        initialize : function() {
            config.IS_IOS = monaca.isIOS;
            config.IS_ANDROID = monaca.isAndroid;
        },
        $get : function() {
            return {
                _getOrientation : function(image) {
                    var orientation = 0;
                    if (config.IS_IOS) {
                        orientation = EXIF.getTag(image, "Orientation");
                    } else if (config.IS_ANDROID){
                        var head = 0;
                         var img = atob(image.src.split(',')[1]);
            while (1){
                    console.log(EXIF.getTag(image, "Orientation"));
...
app.config(function(mobileProvider) {
